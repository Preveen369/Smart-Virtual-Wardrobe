...
const FavoritesPage = ({ isDarkMode }) => {
  ...
  const [favoriteStyleFeed, setFavoriteStyleFeed] = useState([]);
  ...
  useEffect(() => {
    const loadFavorites = async () => {
      setLoading(true);
      try {
        const all = await favoritesService.list();
        // separate by type
        const tryon = all.filter((f) => f.type === "tryon");
        const wardrobe = all.filter((f) => f.type === "wardrobe");
        const stylefeed = all.filter((f) => f.type === "stylefeed");
        // extract item and keep favId for later removals
        setFavoriteTryon(tryon.map((f) => ({ ...f.item, favId: f.id })));
        setFavoriteWardrobe(wardrobe.map((f) => ({ ...f.item, favId: f.id })));
        setFavoriteStyleFeed(stylefeed.map((f) => ({ ...f.item, favId: f.id })));
      } catch (err) {
  ...
  if (loading)
    return (
      <div className="text-center mt-20" style={{ color: textColor }}>
        Loading...
      </div>
    );
  ...
  const removeTryonFavorite = async (favId) => {
    try {
      await favoritesService.delete(favId);
      setFavoriteTryon((prev) => prev.filter((item) => item.favId !== favId));
    } catch (err) {
      console.error("Failed to delete try-on favorite", err);
    }
  };
  // Remove a wardrobe favorite
  const removeWardrobeFavorite = async (favId) => {
    try {
      await favoritesService.delete(favId);
      setFavoriteWardrobe((prev) => prev.filter((item) => item.favId !== favId));
    } catch (err) {
      console.error("Failed to delete wardrobe favorite", err);
    }
  };
  // Remove a stylefeed favorite
  const removeStyleFeedFavorite = async (favId) => {
    try {
      await favoritesService.delete(favId);
      setFavoriteStyleFeed((prev) => prev.filter((item) => item.favId !== favId));
    } catch (err) {
      console.error("Failed to delete stylefeed favorite", err);
    }
  };
        ...
        >
          ...
          <div>
            <Row gutter={[32, 32]}>
                          ...
                          <Card
                            ...
                            cover={
                              ...
                              >
                                ...
                                <HeartFilled
                                  onClick={() =>
                                    removeWardrobeFavorite(item.favId)
                                  }
                                  style={{
                                    position: "absolute",
                                    top: 14,
              ...
              <Col xs={24} md={12}>
                <div style={{ marginBottom: 70, marginRight: 12 }}>
                  <Title
                    level={3}
                    style={{
                      color: textColor,
                          ...
                          <Card
                            ...
                            cover={
                              ...
                              >
                                ...
                                />
                                <HeartFilled
                                  onClick={() => removeTryonFavorite(item.favId)}
                                  style={{
                                    position: "absolute",
                                    top: 14,
                                    right: 14,
          ...
          {/* Favorite Style Feed Images Section */}
          <div className="mt-10 md:mt-20" style={{ marginBottom: 32, marginLeft: 12, marginRight: 12 }}>
            <Title
              level={3}
              style={{
                color: textColor,
                    ...
                    <Card
                      ...
                      cover={
                        ...
                        >
                          ...
                          />
                          <HeartFilled
                            onClick={() => removeStyleFeedFavorite(item.favId)}
                            style={{
                              position: "absolute",
                              top: 14,
                              right: 14,
          ...
          {/* Features Section */}
          <div className="mt-10 md:mt-20" style={{ marginBottom: 38 }}>
            <Title
              level={2}
              style={{
                color: textColor,
